...
export function parseReceivedChain(receivedLines: string[]) {
...
export function detectEspType(headersObj: any, receivedLines: string[]): { type: string; confidence: number; signals: string[] } {
  ...
  if (/google\.com|mail-.*\.google/.test(receivedStr)) scores.Gmail += 1;
  if (/outlook|office365/.test(receivedStr)) scores.Outlook += 1;
  if (/amazonses|amazonses\.com/.test(receivedStr)) scores['Amazon SES'] += 1;
